...
class BackendProxy {
     ...
     */
    async proxyRequest(req, res, backend) {
     ...
     */
    forwardResponseHeaders(backendResponse, clientResponse) {
    ...
    shouldSkipHeader(headerName) {
        ...
        return skipHeaders.some(skip => headerName.toLowerCase().includes(skip.toLowerCase()));
    }
    /**
     * Forward response body from backend to client
     */
    async forwardResponseBody(backendResponse, clientResponse) {
        // Set status code
        clientResponse.status(backendResponse.status);
     ...
     */
    async streamResponse(req, res, backend) {
        const startTime = Date.now();
        try {
            const targetUrl = this.buildTargetUrl(req, backend);
            const headers = this.prepareHeaders(req, backend);
