...
export class CircuitBreaker {
  ...
  private readonly halfOpenRequestCount: Map<string, number> = new Map();
  ...
  constructor(config?: {
    ...
    if (config?.recoveryTimeout) this.recoveryTimeout = config.recoveryTimeout;
    if (config?.halfOpenMaxRequests) this.halfOpenMaxRequests = config.halfOpenMaxRequests;
  }
  ...
  getStats(): {
    ...
    let openCount = 0;
    let halfOpenCount = 0;
    const backendStates: Record<string, any> = {};
