...
export class HealthChecker {
  ...
  async checkHealth(url: string): Promise<HealthStatus> {
    try {
      ...
      if (response.ok) {
        ...
        healthStatus = {
          status: this.determineHealthStatus(healthData as any, responseTime),
          latency: responseTime,
          load: (healthData as any).load || 0,
          timestamp: new Date()
        };
      } else {
        healthStatus = {
   ...
   */
  private determineHealthStatus(healthData: any, latency: number): 'healthy' | 'degraded' | 'unhealthy' {
   ...
   */
  async checkMultipleHealth(urls: string[]): Promise<Map<string, HealthStatus>> {
